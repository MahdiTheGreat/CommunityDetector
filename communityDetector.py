...
def hasher(row, col):
    if row > col:
        return 12 * (col + row) * (col + row + 1) + row
    else:
        return 12 * (col + row) * (col + row + 1) + col
...
def Min(arr):
    ...
    for i in range(1, len(arr)):
        if arr[i].getWeight() == min(res.getWeight(), arr[i].getWeight()):
            res = arr[i]
...
if __name__ == '__main__':
    ...
    edgesMap = dict()
    nodesMap = dict()
    fileName = "RUN Insertion communityTest2.csv"
    mode = fileName.split(" ")
    mode = mode[1]
    csvAndCallTime=[]
    a=[]
    ...
    for i in range(0, len(result)):
        ...
        if not (hasher(result[i][1], result[i][0]) in edgesMap):
            hash = hasher(result[i][1], result[i][0])
            tempEdge = edge(result[i][1], result[i][0], 1)
            if hash in edgesMap:
                print("collision aquired")
            else:
